...
#def plotSpectra(df, lorentz_bool = False):
#    for filename, excEnergy, oscStrength in zip(df['filename'], df['Excitation energy'], df['Osc. strength']):
#        maxEv = math.ceil(max(excEnergy))
#        minEv = math.floor(min(oscStrength))
#
#        x = np.linspace(minEv, maxEv, num=200)
#        y = np.zeros_like(x)
#
#        for x0, scale in zip(excEnergy, oscStrength):
#                if  lorentz_bool:
#                        y += lorentz(x, x0, scale)
#                else:
#                        y += gaussian(x, x0, scale)
#        plt.figure()
#        plt.title(filename)
#        plt.plot(x,y)
#        plt.bar(excEnergy, oscStrength, width=0.1)
#        plt.show()
#        plt.savefig(filename,format='.png')
#        plt.close()
#
#def compareSpectra(df, expExc, expStrength, lorentz_bool=False):
#    for filename, excEnergy, oscStrength in zip(df['filename'], df['Excitation energy'], df['Osc. strength']):
#        maxEv = math.ceil(max(max(excEnergy),max(expExc)))
#        minEv = math.floor(min(min(excEnergy),min(expExc)))
#
#        x = np.linspace(minEv, maxEv, num=200)
#        y = np.zeros_like(x)
#
...
#        plt.plot(x,y)
#        plt.bar(excEnergy, oscStrength, width=0.1)
#        plt.bar(expExc, expStrength, width=0.1)
#
#        plt.show()
#        plt.savefig(filename,format='.png')
#        plt.close()
...
##def plotSpectra(excEnergy, oscStrength, lorentz_bool=False):
##        maxEv = math.ceil(max(excEnergy))
##        minEv = math.floor(min(excEnergy))
##
##        x = np.linspace(minEv, maxEv, num=200)
##        y = np.zeros_like(x)
##
...
##
##
##def compareSpectra(excEnergy, oscStrength, expExc, expStrength, lorentz_bool=False):
##        maxEv = math.ceil(max(max(excEnergy),max(expExc)))
##        minEv = math.floor(min(min(excEnergy),min(expExc)))
##
##        x = np.linspace(minEv, maxEv, num=200)
...
##
#
#def lorentz(x, x0, scale):
#    if np.isnan(x0) or np.isnan(scale):
#        return np.zeros_like(x)
#    elif scale == 0:
#        return np.zeros_like(x)
#    else:
#        return scale / (1 + np.power( (x - x0) / (1/scale), 2)) 
#
#def gaussian(x, x0, scale, std_devi=0.4):
#    if np.isnan(x0) or np.isnan(scale) or scale == 0:
#        return np.zeros_like(x)
#    else:
#        return scale / std_devi * np.exp(-np.power( (x-x0) / std_devi,2))
